...
Name of the file containing the left-end reads.

=item right

Name of the file containing the right-end reads.
...
sub new {
    ...
    open(my $lh, "<$left") || die "Could not open FASTQ file $left: $!";
    $retVal->{lh} = $lh;
    # Store the handle for the right file if we are not interlaced.
    if ($right) {
        open(my $rh, "<$right") || die "Could not open FASTQ file $right: $!";
        $retVal->{rh} = $rh;
    }
